...
|---------|------------|----------|----------|------|
| minimal.bin | 1 byte | 1 | 0.01s | 단순 HLT |
| hello.bin | 33 bytes | 13 | 0.01s | 문자열 출력 (12자) |
| counter.bin | 18 bytes | 11 | 0.01s | 숫자 출력 (0-9) |
| multiplication.bin | 142 bytes | 491 | 0.01s | 구구단 (2-9단) |
| fibonacci.bin | 82 bytes | ~50 | 0.01s | Fibonacci (15개) |
| matrix.bin | 8.3 KB | 93 | 0.01s | 8x8 행렬 곱셈 |
...
```bash
$ /usr/bin/time -v ./kvm-vmm guest/multiplication.bin 2>&1 | grep "Maximum resident"
